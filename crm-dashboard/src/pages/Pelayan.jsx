...
export function Pelayan() {
  ...
  useEffect(() => {
    Promise.all([getPelayanList({ limit: 1000 }), getMembers({ limit: 1000 }), getPelayananInfo()])
      .then(([pelayanData, membersData, pelayananData]) => {
        ...
        pelayananData.forEach(p => {
          idToCol[p.pelayanan_id] = `is_${p.nama_pelayanan.toLowerCase().replace(/\s+/g, '_')}`
        })
        setMinistryRoles(pelayananData.map(p => ({
          key: idToCol[p.pelayanan_id] || `pel_${p.pelayanan_id}`,
